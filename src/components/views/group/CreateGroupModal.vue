<template>
  <BaseModal v-model="modalOpen" size="md" @close="handleClose">
    <template #header>
...
// v-model로 내부에서 관리할 모달 상태
const modalOpen = ref(false);

// props.isOpen이 변경되면 내부 상태 변경
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await createGroup(groupName.value.trim());
